...
- **Episode 0 / ignored seasons** — file-based ignore list for seasons with unwatchable episode-0 placeholders; three-way handling (silent filter, new detection with notification, stale detection)
...
| # | Option | Description |
|---|--------|-------------|
| 1 | **Scrape all series** | Full scrape of all watched series. Choose single-session or multi-session mode. |
| 2 | **Scrape NEW series only** | Scrapes only series not yet in the index (faster). |
| 3 | **Single link / batch add** | Add a single series by URL, or batch-import from a text file. |
| 4 | **Generate report** | Full or filtered report (subscribed / watchlist / both). Saves to JSON. |
| 5 | **Scrape subscribed/watchlist** | Fetch series from your s.to subscription or watchlist pages. |
